    ...
    if ! command -v bun &> /dev/null; then
        ...
        case "$PLATFORM" in
            macOS|Linux)
                ...
                ;;
            Windows)
                echo "🦀🦀  Windows users: Attempting to install Bun via PowerShell..."
...
setup_repo() {
    if is_simpleclaw_dir; then
        ...
        git pull origin main 2>/dev/null || echo "🦀🦀  Could not update (not a git repo or no network)"
    else
...
setup_browser() {
    ...
    if [ "$PLATFORM" = "Windows" ]; then
        ...
        if command -v bun &> /dev/null; then
            bunx playwright install chromium 2>/dev/null || echo "🦀🦀  Playwright installation skipped (optional)"
        elif command -v npx &> /dev/null; then
            npx playwright install chromium 2>/dev/null || echo "🦀🦀  Playwright installation skipped (optional)"
        else
            echo "🦀🦀  Neither bun nor npx available, skipping Playwright"
        ...
        if command -v npx &> /dev/null; then
            ...
            echo "🦀🦀  npx not available, skipping Playwright (browser skill may need manual setup)"
        fi
    fi
}
# Final instructions
