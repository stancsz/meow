...
```typescript
// agent-kernel/src/sidecars/tool-registry.ts
interface Tool {
  name: string
  description: string
...
- [ ] Hot-reload from `.agent-kernel/tools/` (future enhancement)
...
- [x] Basic session store exists (`agent-kernel/src/core/session-store.ts`)
...
- [x] Load from `.agent-kernel/permissions.json` ✅
...
- [ ] Custom commands from `.agent-kernel/commands/`

### 3.2 Task Sidecar ✅
**Problem:** No task tracking.
**Solution:** File-based task store.

- [x] `task-store.ts` exists (`agent-kernel/src/core/task-store.ts`)
- [x] `/add <task>` — add task
- [x] `/done <id>` — complete task
- [x] Task persistence in `.agent-kernel/tasks.json`
...
- [ ] Load skills from `.agent-kernel/skills/`
...
```typescript
// agent-kernel/src/core/loader.ts
interface Sidecar {
  name: string
  init?(): Promise<void>
  tools?(): Tool[]
...
function loadSidecars(): void {
  const dir = '.agent-kernel/sidecars'
...
- **Test path fix** — gap-impl.test.ts uses relative paths, runs from agent-kernel/ directory ✅
...
- **gap-impl.test.ts paths fixed** — tests now use relative paths (`"src/core/lean-agent.ts"`) instead of `"agent-kernel/src/..."` prefix, allowing tests to run from agent-kernel/ directory ✅
- Tests can now be run via `bun test` from the agent-kernel/ directory directly
