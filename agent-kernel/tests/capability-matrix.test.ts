 ...
 *
 * Run with: bun test agent-kernel/tests/capability-matrix.test.ts
 */
...
describe("CORE ENGINE Capability Matrix", () => {
  test("Meow has lean agent loop", () => {
    expect(existsSync("src/core/lean-agent.ts")).toBe(true);
...
describe("TOOLS Capability Matrix", () => {
  test("Meow has Read tool (via tool-registry)", () => {
    const toolRegistrySrc = readFileSync("src/sidecars/tool-registry.ts", "utf-8");
  ...
  test("Meow has Glob tool (sidecar)", () => {
    expect(existsSync("src/tools/search.ts")).toBe(true);
...
describe("TOOL REGISTRY SIDECAR", () => {
  test("Tool registry sidecar exists", () => {
    expect(existsSync("src/sidecars/tool-registry.ts")).toBe(true);
    console.log("  [SIDECAR] tool-registry.ts: EXISTS");
  });

  test("Tool registry has initializeToolRegistry", () => {
    const src = readFileSync("src/sidecars/tool-registry.ts", "utf-8");
...
describe("TASK SYSTEM Capability Matrix", () => {
  test("Meow has task store", () => {
    expect(existsSync("src/core/task-store.ts")).toBe(true);
...
describe("SESSION & STATE Capability Matrix", () => {
  test("Meow has session store (JSONL)", () => {
    expect(existsSync("src/core/session-store.ts")).toBe(true);
...
describe("SKILLS SYSTEM Capability Matrix", () => {
  ...
  test("[GAP-SKILL-001] Dynamic skill loading", () => {
    ...
    const hasDynamic = skillsLoaderSrc.includes("dynamicImport") || skillsLoaderSrc.includes(".agent-kernel/skills/");
...
describe("HOOKS SYSTEM Capability Matrix", () => {
  test("[GAP-HOOK-001] Pre-tool hooks", () => {
    const hasHooks = existsSync("src/sidecars/hooks.ts") || existsSync("src/hooks.ts");
...
describe("MCP Capability Matrix", () => {
  test("[GAP-MCP-001] MCP client implementation", () => {
    const hasMCP = existsSync("src/mcp-client.ts") || existsSync("src/sidecars/mcp-client.ts");
    console.log(`  [GAP-MCP-001] MCP client: ${hasMCP ? 'IMPLEMENTED' : 'MISSING'}`);
    expect(true).toBe(true);
  });

  test("[GAP-MCP-002] MCP server configuration", () => {
    const hasConfig = existsSync(".agent-kernel/mcp.json") || existsSync("mcp.json");
...
describe("SLASH COMMANDS Capability Matrix", () => {
  test("[GAP-SLASH-001] /help command", () => {
    const hasSlash = existsSync("src/sidecars/slash-commands.ts") || existsSync("src/slash-commands.ts");
    console.log(`  [GAP-SLASH-001] /help: ${hasSlash ? 'IMPLEMENTED' : 'MISSING'}`);
    expect(true).toBe(true);
  });

  test("[GAP-SLASH-002] /plan command", () => {
    const hasPlan = existsSync("src/sidecars/plan.ts") || existsSync("src/plan.ts");
  ...
  test("[GAP-SLASH-008] Custom slash commands", () => {
    const hasCustom = existsSync(".agent-kernel/commands") || existsSync("src/commands");
...
describe("UI/TUI Capability Matrix", () => {
  test("[GAP-UI-001] Rich tool rendering", () => {
    const hasTUI = existsSync("src/sidecars/tui.ts") || existsSync("src/tui.ts");
  ...
  test("[GAP-UI-006] REPL mode", () => {
    const hasREPL = existsSync("src/sidecars/repl.ts") || existsSync("src/repl.ts");
