...
describe("GAP-TRACKING: Hooks System", () => {
  ...
  test("GAP-HOOK-001: No hooks implementation", () => {
    const hasHooksFile = existsSync("./agent-kernel/src/sidecars/hooks.ts") ||
                          existsSync("./agent-kernel/src/hooks.ts");
  ...
  test("GAP-HOOK-002: No hooks config file", () => {
    const hasConfig = existsSync(".agent-kernel/hooks.json") ||
                     existsSync("./agent-kernel/src/sidecars/hooks.ts");
...
describe("GAP-TRACKING: MCP", () => {
  ...
  test("GAP-MCP-001: MCP client implementation", () => {
    const hasMCP = existsSync("./agent-kernel/src/mcp-client.ts") ||
                   existsSync("./agent-kernel/src/sidecars/mcp-client.ts");
   ...
   * Priority: P2 - MEDIUM
   * Claude Code: .agent-kernel/mcp.json with server definitions
   * Meow: No config
   * Impact: No MCP server configuration
   */
  test("GAP-MCP-002: No MCP configuration", () => {
    const hasConfig = existsSync(".agent-kernel/mcp.json") ||
                     existsSync("./mcp.json");
    expect(hasConfig).toBe(false);
  });
});
...
describe("GAP-TRACKING: Slash Commands", () => {
  ...
  test("GAP-SLASH-001: No slash command parser", () => {
    const hasSlash = existsSync("./agent-kernel/src/sidecars/slash-commands.ts") ||
                     existsSync("./agent-kernel/src/slash-commands.ts");
  ...
  test("GAP-SLASH-002: No plan mode", () => {
    const hasPlan = existsSync("./agent-kernel/src/sidecars/plan.ts") ||
                    existsSync("./agent-kernel/src/plan.ts");
  ...
  test("GAP-SLASH-004: No /resume command", () => {
    const hasResume = existsSync("./agent-kernel/src/sidecars/slash-commands.ts");
  ...
  test("GAP-SLASH-006: No custom slash commands", () => {
    const hasCustom = existsSync(".agent-kernel/commands") ||
                      existsSync("./agent-kernel/src/commands");
    expect(hasCustom).toBe(false);
  });
});
...
describe("GAP-TRACKING: UI/TUI", () => {
  ...
  test("GAP-UI-001: No rich terminal rendering", () => {
    const hasTUI = existsSync("./agent-kernel/src/sidecars/tui.ts") ||
                   existsSync("./agent-kernel/src/tui.ts");
  ...
  test("GAP-UI-002: No REPL mode", () => {
    const hasREPL = existsSync("./agent-kernel/src/sidecars/repl.ts") ||
                    existsSync("./agent-kernel/src/repl.ts");
  ...
  test("GAP-UI-004: No status bar", () => {
    const hasStatus = existsSync("./agent-kernel/src/sidecars/tui.ts");
