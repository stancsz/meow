...
3. **Hook config** - JSON file at ~/.meow/hooks.json
 ...
 * Minimal slice: Implement --hooks flag: pre-hook runs before command, post-hook runs after. Config in ~/.meow/hooks.json
 */
...
```",
  async execute(context) {
    ...
    // Implement --hooks flag: pre-hook runs before command, post-hook runs after. Config in ~/.meow/hooks.json
