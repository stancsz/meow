...
1. Store `~/.meow/sessions/<id>/checkpoints/` with git commits + conversation snapshots before each tool call
...
4. Key insight: use a shadow git repo in `~/.meow/history/<project_hash>/` (separate from user's repo)
