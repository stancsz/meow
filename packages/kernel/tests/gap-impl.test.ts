 ...
 *
 * Run with: bun test agent-kernel/tests/gap-impl.test.ts
 */
...
describe("GAP-TEST-SYNTAX: Fix string concatenation in gaps.test.ts", () => {
  ...
  test("gaps.test.ts has no malformed string concatenation", () => {
    ...
    // The bug is: existsSync("./agent-kernel/src/"sidecars/hooks.ts");
    // This is invalid JS - strings adjacent without + are a syntax error
    // These should be: existsSync("./agent-kernel/src/sidecars/hooks.ts");
  ...
  test("gaps.test.ts existsSync calls use valid path strings", () => {
    ...
    const validPaths = [
      '"./agent-kernel/src/sidecars/hooks.ts"',
      '"./agent-kernel/src/sidecars/mcp-client.ts"',
      '"./agent-kernel/src/sidecars/slash-commands.ts"',
      '"./agent-kernel/src/sidecars/plan.ts"',
      '"./agent-kernel/src/sidecars/tui.ts"',
      '"./agent-kernel/src/sidecars/repl.ts"',
    ];
  ...
  test("gaps.test.ts parses without syntax errors", () => {
    ...
    // should now be:
    // existsSync("./agent-kernel/src/sidecars/hooks.ts");
    expect(gapsSrc).not.toContain('"./agent-kernel/src/"sidecars');
    expect(gapsSrc).not.toContain('"./agent-kernel/src/"mcp-client');
    expect(gapsSrc).not.toContain('"./agent-kernel/src/"slash-commands');
    expect(gapsSrc).not.toContain('"./agent-kernel/src/"plan');
    expect(gapsSrc).not.toContain('"./agent-kernel/src/"commands"');
    expect(gapsSrc).not.toContain('"./agent-kernel/src/"tui');
    expect(gapsSrc).not.toContain('"./agent-kernel/src/"repl');
  });
});
