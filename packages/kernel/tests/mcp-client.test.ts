 ...
 *
 * Run with: bun test agent-kernel/tests/mcp-client.test.ts
...
// Test files live in agent-kernel/tests/, project root is the parent of "agent-kernel/"
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const PROJECT_ROOT = dirname(__dirname); // = agent-kernel/ directory
...
║    ✅ Config loading from ~/.agent-kernel/mcp.json                                   ║
...
describe("MCP Live Integration (GAP-MCP-01)", () => {
  // Path to the mock MCP server script (relative to agent-kernel/ project root)
