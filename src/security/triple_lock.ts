...
export const enforceSecurityLocks = (req: Request): Response | null => {
  ...
  if (!process.pid) {
    return new Response(JSON.stringify({ error: "Security Lock 1 Failed: PID missing" }), {
      status: 403,
  ...
  if (!spiffeId) {
    return new Response(JSON.stringify({ error: "Security Lock 2 Failed: Missing x-agent-id" }), {
      status: 403,
      headers: { "Content-Type": "application/json" }
    });
  }
