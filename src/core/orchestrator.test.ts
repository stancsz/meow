...
describe("Orchestrator Cloud Function (Execution Flow)", () => {
    test("handles action: 'approve' and correctly dispatches workers via executeSwarmManifest", async () => {
        // Setup a local test DB to simulate motherboard
        const originalDbUrl = process.env.DATABASE_URL;
        process.env.DATABASE_URL = "sqlite://local_test_db_orchestrator_execute.sqlite";
        const { DBClient } = require("../db/client");
        const fs = require("fs");
        const testDb = new DBClient(process.env.DATABASE_URL);
        ...
        // Prepare a valid manifest that requires a mock-skill
        const manifest: SwarmManifest = {
            version: "1.0",
            intent_parsed: "Test execution flow",
        ...
        // Create a session to attach the manifest
        const sessionId = testDb.createSession("user_exec_test", { prompt: "exec test" }, manifest);
        ...
        // Ensure to clear NEXT_PUBLIC_API_URL so it falls back to direct execution in executeSwarmManifest
        const originalApiUrl = process.env.NEXT_PUBLIC_API_URL;
        process.env.NEXT_PUBLIC_API_URL = "";

        // Trigger orchestratorHandler with approve action
        await orchestratorHandler(req, res);

        // Check immediate synchronous response
        expect(statusCode).toBe(200);
        expect(responseBody.status).toBe("dispatched");
        expect(responseBody.executionId).toBe(sessionId);

        // Wait a brief moment to let the asynchronous executeSwarmManifest complete its writes
        await new Promise(resolve => setTimeout(resolve, 100));

        // Read the updated session to verify status change and audit logs
        const updatedSession = testDb.getSession(sessionId);
        expect(updatedSession.status).toBe("completed");

        const testDbAny = testDb as any;
        const taskResults = testDbAny.db.query("SELECT * FROM task_results WHERE session_id = ?").all(sessionId);
