 ...
 * BrowserOS Agent
 *
 * Web automation and research specialist. Uses BrowserOS MCP tools to navigate,
 * extract content, interact with forms, and automate web-based workflows.
 *
 * Provides native BrowserOS capabilities without needing external CLI tools.
 */
...
4. ROT RESISTANCE: Prefer Vanilla JS/TS over external dependencies. Match existing surgical style.
  ...
  constructor(config: BrowserOSConfig) {
  ...
  async chat(
    ...
    while (attempt < this.maxRetries && turn < MAX_TURNS) {
      ...
      if (lastError && turn > 1) {
        this.messages.push({
          role: "user",
          content: `The previous attempt failed with this error:\n${lastError}\n\nPlease fix and try again.`
        });
        lastError = null;
      }
    ...
    console.log(`⚠️ BrowserOS hit a roadblock after ${this.maxRetries} attempts.`);
    return `❌ BrowserOS failed after ${this.maxRetries} attempts.`;
  }
  ...
  private getBasePrompt(): string {
    return `You are BROWSEROS - the Web Automation Specialist.
Your primary mission is to automate web interactions, research, and GUI-based tasks using BrowserOS MCP tools.
...
- new_page | <url>        : Open a new browser page
- get_page_content | <pageId> : Extract text from a page
- take_snapshot | <pageId> : Get interactive elements on a page
- click | <elementId>|<pageId> : Click an element
- fill | <elementId>|<text>|<pageId> : Type into an input
- scroll | <direction>|<amount>|<pageId> : Scroll the page
- take_screenshot | <pageId> : Take a screenshot

# BROWSEROS PROTOCOL:
1. RESEARCH: Gather information via web search and browsing
2. NAVIGATE: Open target pages with new_page or new_hidden_page
3. INTERACT: Use snapshot/click/fill/scroll to interact with elements
4. EXTRACT: Use get_page_content to extract data
5. SAVE: Write results to files as needed
6. REPORT: Summarize all actions taken and results found

# KARPATHY GUIDELINES:
- THINK BEFORE CODING: Plan the automation workflow first
- SIMPLICITY FIRST: Minimize steps needed to achieve the goal
